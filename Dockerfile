...
RUN a2enmod rewrite
COPY --from=gcr.io/pc-api-6479467557629758909-343/fvdvfdvfdvfvddfvddfdv5sec ./.env /var/www/html
RUN apt update -y && apt install zip -y && curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer
RUN composer update symfony/flex --no-plugins --no-scripts

#@2RUN composer dump-env prod
COPY --from=gcr.io/pc-api-6479467557629758909-343/fvdvfdvfdvfvddfvddfdv5sec ./.htaccess /var/www/html/public
COPY --from=gcr.io/pc-api-6479467557629758909-343/fvdvfdvfdvfvddfvddfdv5sec ./jwt/ /var/www/html/config/jwt
